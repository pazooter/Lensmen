...
// ── LENSES (13 original + 3 new Data Series outpoints) ───────────────────────
const LENSES = [
...
export default function Lensmen() {
  ...
  // idle | ideal_scene_loading | ideal_scene_ready
  ...
  const reset = () => {
    setStage("idle"); setIdealScene(""); setDataScope("");
  ...
  const handleSubmit = async () => {
    ...
    try {
      const scene = await generateIdealScene(claim.trim());
  ...
  const handleConfirmIdealScene = async () => {
    ...
    try {
      const scope = await generateDataScope(claim.trim(), idealScene);
  ...
  const handleConfirmDataScope = async () => {
    ...
    try {
      const data = await runLensAnalysis(claim.trim(), idealScene, dataScope, activeLenses);
      setResult(data);
      // Now derive primary nuttiness
      setStage("primary_loading");
      try {
        const pn = await generatePrimaryNuttiness(data.findings, idealScene, claim);
        setPrimaryNuttiness(pn);
      } catch (_) { /* non-fatal — show results without primary nuttiness */ }
      setStage("done");
  ...
  const handlePursue = async (thread) => {
    ...
    try {
      const scope = await generatePursueScope(claim, idealScene, thread);
      const relevantLenses = await selectRelevantLenses(thread, `Claim: ${claim}\nIdeal Scene: ${idealScene}\nThread: ${thread}`);
  ...
  const busy = ["ideal_scene_loading","data_scope_loading","analysis_loading","primary_loading"].includes(stage);
  const busyLabel = {
  ...
  return (
    <div style={{ maxWidth: 800, margin: "0 auto", padding: "2rem 1rem", fontFamily: "system-ui, sans-serif", color: "#1a1a18" }}>
      ...
      {/* Status during primary nuttiness calculation */}
      ...
      {stage === "done" && result && (
        <div style={{ marginTop: "1.5rem" }}>
          ...
          <div style={{ marginTop: 24, borderTop: "0.5px solid #d3d1c7", paddingTop: 16 }}>
            ...
            {/* New thread input — always visible at top so user can open threads freely */}
            ...
            {/* All threads rendered in parallel — labeled and numbered */}
            {pursues.length > 0 && (
              <div>
